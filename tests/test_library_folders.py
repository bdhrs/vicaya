...
def test_ocr_worker_extracts_scanned_pdf(tmp_path, monkeypatch):
    ...
    payload = library_folders._ocr_worker_main(str(tmp_path / "scan.pdf"))
...
def test_ocr_fallback_spawns_worker_and_returns_ok(tmp_path, monkeypatch):
    ...
    def fake_run(cmd: list[str], **_kwargs):
        ...
        return subprocess.CompletedProcess(
            cmd, 0, stdout='{"status": "ok", "text": "ocr text"}\n', stderr=""
        )

    monkeypatch.setattr(library_folders.subprocess, "run", fake_run)
    monkeypatch.setattr(
        library_folders,
        "_extract_pdf_pdftotext",
        lambda _path: library_folders.ExtractedText(text="", status="empty"),
...
def test_ocr_fallback_timeout(tmp_path, monkeypatch):
    def fake_run(_cmd, **_kwargs):
        raise subprocess.TimeoutExpired(cmd="ocr", timeout=600)
...
def test_ocr_fallback_unsupported_payload_keeps_original_status(tmp_path, monkeypatch):
    def fake_run(cmd, **_kwargs):
        return subprocess.CompletedProcess(
            cmd,
            0,
            stdout='{"status": "unsupported", "detail": "no ocr api"}\n',
            stderr="",
        )
...
def test_ocr_fallback_junk_payload_keeps_original_status(tmp_path, monkeypatch):
    def fake_run(cmd, **_kwargs):
    ...
    assert extracted.status == "error: pdf ocr worker returned junk"
    assert extracted.text == ""
