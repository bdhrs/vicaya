...
- **Other canons:** `search_chinese()` over a local CBETA clone and `search_84000()` over a local 84000 clone — stdlib regex tag-stripping of TEI XML, no index. See **CBETA and 84000 (other canons)** below.
...
## CBETA and 84000 (other canons)

Two optional local corpora answer "what do other schools say about X". Both are plain `git clone --depth 1` folders read straight from their TEI XML at search time; there is no index or cache to build or go stale. Agent procedure lives in Phase 3b of `skill/vicaya/SKILL.md`. All figures below were measured on 2026-09-24 (thread `20260924_cbeta_84000_sources`, test scripts in its `artifacts/`).

- **CBETA** (`VICAYA_CBETA_PATH`, clone of `cbeta-org/xml-p5`, 2.6 GB, CC BY-NC). One file per text per volume (`T/T02/T02n0099.xml`); `T0220` spans three volumes and `search_chinese` merges them under one id. `<lb n="0120a06"/>` opens a Taishō line; lines are joined before matching because phrases cross them, and each hit reports the line it starts on. `<note>`, `<rdg>`, header and back matter are dropped. Rare characters (`<g ref="#CB…">`) resolve from each file's own `<charDecl>` — parse each `<char>` block separately: one regex spanning blocks runs into the neighbour and leaves characters unresolved. All 111,343 in the Taishō resolve.
- **Why a process pool, not a cache:** a Taishō-wide search for 涅槃 took 13.9 s on one core and 2.4–2.7 s at 12 workers, with identical hit counts (56,626); after the review's clean-up passes (comments, Siddham, per-line unescape) it is 3.0 s, and `--collection all` (5,017 files) 6.7 s. Workers are `min(12, os.cpu_count())`, the same headroom-over-peak choice as `OCR_JOBS`. The pool uses the `forkserver` start method: the Linux `fork` default raised 60 "multi-threaded … fork() may lead to deadlocks" warnings under pytest, and forkserver measured the same speed (about 2.6 s).
- **84000** (`VICAYA_84000_PATH`, clone of `84000/data-tei`, 250 MB, CC BY-NC-ND 3.0). Published translations are `translations/kangyur/translations/` (396) and `translations/tengyur/publications/` (3); `placeholders/` are stubs and are skipped. Toh numbers come from the filename (`…_toh540,1078-…`), because the in-file `<bibl key>` is missing in 3 files. A single-process paragraph search over all 399 files takes about 3.0 s. Reader links are `https://84000.co/translation/toh<N>` (the old `read.84000.co/….html` form 301-redirects there).
- Both return `{query, total_hits, total_texts, by_text, hits}` — per-text counts first (capped at 50; `total_texts` counts all), because common terms return tens of thousands of hits and the counts tell the agent which text to open with `--text` / `--toh`. Bad input (unknown collection, a text id matching no file, an empty query) raises `ValueError` and the CLI exits 1: a zero-hit result must mean a genuine absence.
- CBETA traps, each found on real data by the thread review: real files write rare characters as `<g ref="#CB…">glyph</g>` (the glyph is often private-use), so the whole element is replaced, not just the opening tag; X files carry reprint-edition `<lb ed="R150">` tags, so only the file's own edition numbers lines; XML comments hold inner tags and must be dropped before tag-stripping; dhāraṇīs interleave Siddham glyphs (no Unicode) with the Chinese syllables, so `<cb:t xml:lang="sa…">` and `SD-`/`RJ-` glyphs are dropped. After these, the whole Taishō flattens with 0 stray `<`/`>` and 0 private-use characters.
- 84000 traps: endnote anchors are self-closing `<note …/>`, so the note-stripper must refuse to cross another `<note` (a plain `<note>.*?</note>` swallowed 35 KB of Toh 99); hits carry `section` (`front` summary/introduction, `body` translation, `back` apparatus); glossary definitions live in `<note type="definition">` and are not searched.
- The DPD database holds no Chinese, so Chinese search terms come from the Āgama parallel text, WisdomLib/web, or the agent's own knowledge.
...
Set for headroom, not for peak throughput — the machine is the user's workstation. Two traps when reading CPU during a run: ocrmypdf **nices** its workers, so the load appears in `top`'s `ni` column and a monitor showing user CPU only will read 2–3 % while eight cores are saturated; and a stray ocrmypdf from an earlier run doubles the real worker count, making the setting look far too high — that was a real orphan-on-stop bug, now fixed, but check for strays before re-tuning anyway.
...
**The same isolation has a cost, and it is closed by explicit cleanup:** because the child has its own session, a Ctrl-C or `kill` on the refresh does not reach it on its own. So `_extract_pdf_ocr_fallback` kills the group on **any** interruption of its wait, not only on a timeout, and wraps that wait in a SIGTERM handler that raises — Python's default SIGTERM handler exits the process without unwinding, which would skip the cleanup a Ctrl-C runs. Verified by `test_stopping_a_refresh_kills_the_ocr_child`, which sends a real SIGINT and a real SIGTERM to a real refresh and requires the OCR child dead; it fails for both signals when either half of the cleanup is removed. Before this was fixed (2026-09-04) a stopped refresh left ocrmypdf running at `OCR_JOBS` cores, twice — once alongside a restarted run on the same book, which saturated the machine at 20 workers on 22 cores and read as "`--jobs 12` is too high" when it was not.
