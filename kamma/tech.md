...
(`tools/library_folders.py::_ocr_worker_main`) so an engine hang cannot stall
a refresh: it is killed after `OCR_SUBPROCESS_TIMEOUT` (1800 s) and recorded as
an error status. Work per file is additionally capped at the first
