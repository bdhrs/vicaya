...
- Cross-check uses `VICAYA_CROSS_CHECK_CHAIN` (env), a pipe-separated `app:model` list tried in order (`opencode run -m <model>`, `agy --print ... --model <model>`, or `pi -p --model <provider/id> ...`). The first successful subprocess response wins; if the chain is empty or every entry fails, the helper returns a `# SELF_REVIEW:` sentinel whose header names **exactly which entry failed and why** (timeout / non-zero exit / could not start / no output) — fix the named cause and retry once before accepting self-review. `cross-check --preflight --timeout 60` probes every entry with a tiny prompt before the long wait. vicaya does not store any provider API keys — the app (`opencode`, `agy`, or `pi`) must already be authenticated; `pi` entries use pi's own key store (`pi auth check --provider zai`), so `pi:zai/glm-5.3` runs on the original Z.ai API, not OpenRouter. **Pin a thinking level on pi entries** (`pi:zai/glm-5.3:off`) — pi's `--model` accepts the `:thinking` suffix natively, and an unpinned entry inherits the machine's `defaultThinkingLevel` (often `max` = a 10+ minute review); `:off` keeps a full review in the ~1–2 min range while still catching tier and citation errors (measured 2026-08-17: glm-5.3:off 87 s, flash:off 32 s, vs 12–17 min at inherited max).
...
| `cross-check` | `--timeout N` `--preflight` `--quiet`; **prompt on stdin** (preflight reads none) | App/model chain per `VICAYA_CROSS_CHECK_CHAIN` → `# SELF_REVIEW:` sentinel on failure, with a header naming which entry failed and why. `--preflight --timeout 60` probes each entry with a tiny prompt first. Output is post-processed: every sutta citation is stamped `[VERIFIED]`, `[REJECTED — not in sutta_info]`, or `[UNVERIFIABLE — …]` (global verse numbers). Use this in Phase 6. |
...
- **Citation**: `machine` (e.g. `s0201m_mul:23`), `human` (e.g. `MN60 Apaṇṇakasuttaṃ para 97`), `pitaka`, `text_type`, `paranum`, `paranum_exists` (`true` = the book's table has a row for this paranum; `false` = it has none, so the reference is bogus and the command exits 1; `null` = could not be checked). Never cite a `false`.
...
| Tier | Sources | Weight |
|------|---------|--------|
| **T1a — Root canon (Early Buddhist Texts)** | The four Nikāyas in full (all of `s01*_mul`–`s04*_mul`: DN, MN, SN, AN), the Vinaya Suttavibhaṅga only (Pārājika + Pācittiya: `vin01m_mul`, `vin02m1_mul`), and the earliest Khuddaka Nikāya texts — Khuddakapāṭha, Dhammapada, Udāna, Itivuttaka, Suttanipāta, Theragāthā, Therīgāthā. This is the same EBT list the sister dpd-db project uses for its own word-frequency calculations (`tools/pali_text_files.py: ebts`) — treat it as authoritative rather than guessing by genre. | Highest confidence for reconstructing the earliest stratum of teaching. Verbatim blockquote mandatory (Rule P1). |
| **T1b — Root canon (later mūla, non-EBT)** | Everything else canonical but outside the T1a list: the Vinaya Khandhakas (Mahāvagga, Cullavagga) and Parivāra, the later Khuddaka texts (Vimānavatthu, Petavatthu, Jātaka, Apadāna, Buddhavaṃsa, Cariyāpiṭaka, Milindapañha, Petakopadesa), and the full Abhidhamma piṭaka (`abh*_mul`). Canonical (part of the Tipiṭaka) but demonstrably later composition than T1a. | Still root-canon citation weight, but never present T1b material as though it were the Buddha's earliest words. When a doctrinal claim turns on the EBT/non-EBT distinction, name the tier explicitly in the citation (e.g. "Jātaka 1, a later narrative addition — T1b"). |
| **T2 — Canonical exegesis** | Aṭṭhakathā (`*_att`), ṭīkā (`*_tik`), Visuddhimagga, and the Khuddaka texts that are exegetical/analytical in genre despite being canonically root-classed: Niddesa, Paṭisambhidāmagga, Nettippakaraṇa | Authoritative within the tradition. Attribute to Buddhaghosa / Dhammapāla / the commentarial tradition — never to the Buddha. |
...
The note template's `## Canon Evidence (T1)` heading covers both T1a and T1b — it is not split into two sections, so a full-canon search still lands in one place. Within that section, a T1a citation needs no tag (it's the default read of "T1"); tag a T1b citation inline with `(T1b)` right after its reference so the reader can spot the EBT/non-EBT boundary without cross-referencing the tier table (issue #40 — this also answers the "nrf-table" classification gap: Milindapañha is T1b, not T1a). The Devil's Advocate checklist question 5 checks that load-bearing claims rest on T1 (either sub-tier) or T2, not T4 — but a claim about the *earliest* teaching on a topic must rest on T1a specifically, not T1b.
...
After the Phase 1 gate passes, run the gather phases as **single-phase sub-agents** — one per applicable phase (2, 2.5, 3, 3b, 4a, 4b, 4c), skipping any phase the angle triage marked non-applicable. Sequential dispatch (spawn, spot-check, spawn next) is the default shape. This keeps the main context clear for synthesis while bounding each sub-agent to one phase.

**Parallel dispatch is sanctioned** (issue #102). The gather phases are independent source domains and helper writes are lock-serialized, so you may dispatch the applicable gather agents as one concurrent wave instead of a sequence — three real runs did so with zero misfiling, zero `phase-source: run-pointer` leaks, and substantial wall-clock savings. Three rules keep it correct: (1) **Gates are still written ascending** — an agent whose phase finishes while an earlier phase is ungated simply waits; backfill the gates in order afterwards (one cheap pass). (2) **Spot-checks still run per agent** — batched after the wave returns instead of between spawns: content, `phase-source: run-pointer` markers, and the top-citation re-verify, every agent, before synthesis builds on any of it. (3) **One agent per phase still holds** — a parallel wave is not permission for one agent to run several phases (see "Why per-phase" below).

**Thematic auto-skip applies to gates only, not to work — and to exactly two gates.** On a thematic (non-sutta-anchored) run, exactly **2.5 and 3b auto-skip**; every other gather gate (1, 2, 3, 4, 4b, 4c) is gated normally. The auto-skip means `scratch-gate` writes the 2.5/3b gates for you — without requiring the usual evidence — when a *later* phase gates, and it means an **explicit** `scratch-gate 2.5` / `scratch-gate 3b` call still demands logged evidence like any content gate (issue #105: agents called these explicitly from muscle memory and hit the refusal). Auto-skipping a gate is not permission to skip the research: if angle 16 (Āgama comparison) or angle 7 (Sanskrit/comparative) was marked *applicable* in Phase 1, run the SC-parallels or Sanskrit searches and log the hits — the gate just won't block on them. And because 4b/4c are *not* auto-skipped, a thematic run must still gate them explicitly: gating Phase 4 does not gate 4a/4b/4c.
...
**After each agent returns — spot-check before spawning the next** (on a parallel wave: after the wave returns, before synthesis). `scratch-verify` now also flags gated-but-empty and placeholder-only phases (`content_issues`), so it backstops a crashed/limited/stubbed agent structurally — but still spot-check immediately after each agent (or after the wave) so a silent gap is caught before the next agent builds on it, not only at the Phase 5 verify.
...
- **Never trust the completion notification's own phase ID or status claim.** A sub-agent's notification summary can cross-label — e.g. a Phase 2 agent's notification reporting Phase 3 status — even when the actual work is filed correctly. Confirm ground truth from `grep -n '^## Phase' <scratch>` or `scratch-resume` output, never from the notification text alone (issue #53).
...
**Harness fallbacks (pi and other non-Claude-Code hosts).** Repeated real runs on pi converged on the same adaptations — use them directly instead of re-deriving:

- **Inline gather is the default on pi.** Its generic named agents (`planner`/`reviewer`/`scout`/`worker`) don't carry the vicaya dispatch prompt reliably, and its `subagent` tool has aborted or hit a ~120s cap mid-phase. Run each gather phase inline in the orchestrator: pin `VICAYA_PHASE=<n>` inline on every helper call, pass `--quiet` on every search, prefer targeted id-window SQL pulls over broad stem dumps, and gate each phase before starting the next. Watch the context budget — inline is fine for a bounded single-term question; a broad multi-sutta topic will overflow, so trim search breadth or split the run.
...
- **Run the cross-check; never pre-assume the SELF_REVIEW fallback** (issue #103: the chain is configured and returns real reviews on this setup — a run that assumed the sentinel nearly skipped a genuine review). **Preflight first when a chain is configured:** `printf 'probe' | uv run tools/research_sources.py cross-check --preflight --timeout 60` — a few seconds, per-entry status JSON; on failure the named reason tells you what to fix (auth, missing binary, latency) and you can go straight to the checklist without burning a 180s wait. If the full call does return `# SELF_REVIEW:`, its header names the exact entry and failure — fix that, retry **once**, then run the checklist. **Never re-invoke cross-check after it already returned a substantive review** — a re-run can clobber the real review with the sentinel; the first successful output is the review. On hosts where none of the chain apps (`opencode`/`agy`/`pi`) are genuinely installed or authenticated, the sentinel is the expected outcome — run the embedded checklist per Phase 6 and record `cross-check: self-review`; it is not a failure.
...
**`paranum_exists` — check it before you cite.** Every naming path resolves by *nearest preceding* heading or sutta_info row, so the output tells you which sutta a paragraph falls inside, not that the paragraph is real. The `paranum_exists` field closes that gap: `true` means the book's table has a row for that paranum, `false` means it has none (the `human` field then reads `NO SUCH PARANUM …` and the command exits 1 — never cite it; search the book for the passage you actually mean, or re-check the book code with `lookup-book`), and `null` means it could not be checked (no `VICAYA_CANON_DB`, or the book has no table there) — unverifiable, not wrong. Before this check a paranum that exists nowhere still produced a confident label: `resolve-citation e0102n_mul 84` returned "Extra 0102 §84 — Visuddhimaggo" for a paranum with zero rows, and a sub-agent reported it as a real citation.
...
→ **Phase 2.5 exit:** `scratch-gate 2.5`. *(Thematic runs: do NOT call this gate explicitly — it auto-skips, and an explicit call demands logged evidence. The work still runs when angle 16 is applicable; the gate just won't block on it.)*
...
Snippets come back with each hit when FTS text is indexed — quote them with book + author attribution. Any status *starting* with `ok` means the snippet is usable. Only fall back to a manual `pdftotext` or `ebook-convert` extraction when the status is `empty`, `unsupported:` or `error:`, and never for a scanned PDF whose status mentions `ocr`: pdftotext is exactly what already returned nothing there, so it is a dead end — try `ocrmypdf --sidecar` instead if you must extract by hand.

**Scanned books lose Pāḷi diacritics** — a quote may read `Akasagarbha` where the page says `Ākāśagarbha`. Restore the diacritics from the research (DPD, the canon DB, the term's known spelling) and mark the quote as restored.
...
- **Extraction is a build-time concern, not a query-time concern.** All formats (PDF, EPUB, MOBI, DOC, etc.) are extracted into the FTS index during `library-folders-refresh`. Read `extraction_status` by prefix, not by equality:
    ...
    - `ok: …` — a success with a note, text is indexed and searchable. Treat the hit as real; mention the note in Critical Gaps only if the answer depends on it.
    - `empty`, `unsupported: …`, `error: …` — nothing is indexed. That is a refresh gap, not something to fix at query time; note it in Critical Gaps. Do not hand-run `pdftotext` on a scanned PDF whose status mentions `ocr` — pdftotext returning nothing is why OCR ran in the first place. `error: pdf ocr output implausibly short …` is a book the OCR engine could not read at all (a reproduced tesseract failure on some scans); there is no second engine, so treat it as a genuine gap in the library rather than something a retry will fix.
...
5. **Evidence tier of load-bearing claims.** Is the central claim of the Findings section supported by T1 (canon mūla) or T2 (commentary) evidence, or does it ultimately rest on a modern teacher's talk or a secondary source? If the latter, the Findings prose must reflect that epistemic status. If the claim is specifically about the *earliest* teaching on a topic, check further: is the T1 support T1a (EBT) or T1b (later mūla, e.g. Jātaka, Apadāna, Abhidhamma)? A T1b-only claim cannot honestly be framed as "what the Buddha taught" without qualification.
...
uv run tools/research_sources.py cross-check --preflight --timeout 60 || true   # cheap reachability probe; read the per-entry status
...
**Preflight before the long wait** (issue #103): the `--preflight` line above probes every `VICAYA_CROSS_CHECK_CHAIN` entry with a tiny prompt under a short budget and prints per-entry status JSON. If it fails, the named reason (timeout / non-zero exit / could not start / no output) tells you what to fix — and if you can't fix it, go straight to the checklist instead of waiting out the full `--timeout` per entry. **Never re-invoke `cross-check` after it has already returned a substantive review** — a re-run that fails would overwrite the real review file with the sentinel; the first successful output is the review (a real run lost a genuine review this way).
...
**Run this in the background from the first attempt** (e.g. `run_in_background: true` on the Bash call), not only after a foreground timeout. The helper's own `--timeout` defaults to 180s, longer than the Bash tool's ~120s foreground cap — on a long synthesis, chain-entry (opencode/agy/pi) latency alone silently truncates the call before `$CROSS_CHECK_REVIEW` is written, even though the underlying model request would have succeeded.

Two timing/shell gotchas: (1) `--timeout` bounds each chain entry, not the whole call — with N entries in `VICAYA_CROSS_CHECK_CHAIN`, total wall-clock can reach N× the value before the sentinel appears, so budget the background wait accordingly. (2) When wiring your own redirects around the backgrounded call, redirect once — a chained `>file >/dev/null 2>&1` sends everything to `/dev/null`, and the empty review file then looks exactly like a SELF_REVIEW fallback.

The `cross-check` helper tries each `app:model` entry in `VICAYA_CROSS_CHECK_CHAIN` (env) in order via subprocess (`opencode run -m <model>`, `agy --print ... --model <model>`, or `pi -p --model <provider/id> ...`), returns the first one that succeeds, and falls back to the `# SELF_REVIEW:` sentinel if the chain is empty or every entry fails. **If the scratch-local review file begins with `# SELF_REVIEW:`**, read its header: it names either that the chain is not configured, or exactly which entry failed and why — fix the named cause and retry **once**; only then proceed to self-review. To self-review, run the embedded five-point checklist on your own synthesis: read each numbered item, audit your synthesis against it, and apply fixes the same way you would for an external review. Do not write anything in the note acknowledging the self-review fallback; it is still subject to the IRON RULE below. The terminal report in Phase 7 records `cross-check: self-review` instead of a model name.
...
- **`search-library-folders` hits come back with `source_available: null`**: the volume holding the library could not be reached (drive not mounted, network share offline). The index still holds the extracted text, so the snippets and `document_id`s are usable and citable as before — what you cannot do is open the original file for a fuller extraction. Mount the volume and re-run if you need the full document; do not report these books as missing from the library. Before 2026-08-10 this case statted every candidate row in turn and blocked for minutes with no diagnostic (issue #92); each root is now probed once with a short deadline.
...
- **`cross-check` returns `# SELF_REVIEW:`**: every configured chain entry failed (or `VICAYA_CROSS_CHECK_CHAIN` is unset/blank). Run the embedded checklist on your own synthesis as described in Phase 6; do not retry the helper. Common root causes: the chain var is empty, no chain app (`opencode`/`agy`/`pi`) is on `$PATH`, or none has valid provider credentials configured. On hosts without any of those apps this is the expected outcome, not an error to debug.
...
- **PDF URL — WebFetch returns garbled or empty content**: WebFetch cannot decode PDF binary. Instead, save the file under this run's repo-local temp directory and extract with `pdftotext`:
  ...
  ```
  `pdftotext` is at `/usr/bin/pdftotext` (Poppler 24.02) and handles Unicode including Pāḷi diacritics. If the PDF is password-protected or corrupt, `pdftotext` will exit non-zero — note the gap and move on. If it exits 0 but returns nothing, the PDF is a scan: `ocrmypdf --force-ocr --sidecar out.txt in.pdf out.pdf` (`/usr/bin/ocrmypdf` 15.2.0, wrapping tesseract 5.3.4) will OCR it, at roughly half a second per page. Its output carries **no Pāḷi diacritics**, so treat any Pāḷi it produces as an approximate spelling, never a verbatim quote.
