...
Snippets come back with each hit when FTS text is indexed — quote them with book + author attribution. Any status *starting* with `ok` means the snippet is usable, including `ok: ocr truncated at 1000 of 1678 pages` (a scanned book OCR'd up to the page cap). A `partial:` status also has usable text — just less of it. Only fall back to a manual `pdftotext` or `ebook-convert` extraction when the status is `empty`, `unsupported:` or `error:`, and never for a scanned PDF whose status mentions `ocr`: pdftotext is exactly what already returned nothing there, so it is a dead end.
...
- **Extraction is a build-time concern, not a query-time concern.** All formats (PDF, EPUB, MOBI, DOC, etc.) are extracted into the FTS index during `library-folders-refresh`. Read `extraction_status` by prefix, not by equality:
    ...
    - `ok: …` — a success with a note, text is indexed and searchable. `ok: ocr truncated at 1000 of 1678 pages` means a scanned book was OCR'd to the page cap: the first 1,000 pages are searchable and quotable, the rest are not indexed. Treat the hit as real; mention the limit in Critical Gaps only if the answer depends on the unindexed part.
