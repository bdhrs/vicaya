...
Write the Phase 5 and Phase 6 gates only by calling the routed canonical `scratch-gate` helper, never as manually written gate headers — bare headers are not honoured by later gate checks and force a backfill in the next stage.
